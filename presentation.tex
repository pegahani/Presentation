...
%\Fontvi
%The goal is to compute an approximated set of non-dominated policies. 
%
%\begin{columns}
%\begin{column}{.40\textwidth}
%\ssmall{
%\alert{Example}: If this set is $\mathcal{V}$,
%\begin{itemize}
%\item  Sophie's optimal trajectory is inside $\mathcal{V}$ (Eco friend)
%\item  Kevin's optimal trajectory is inside $\mathcal{V}$ too (Speed lover)
%\end{itemize}
%}
%\begin{center}
%\includegraphics[scale=0.4]{figures-new/user-choise}
%\end{center}
%{\color{red} figure should be modifies with several optimal trajectories and trajectories for Sophie and Kevin}
%\end{column}
%\hspace{-0.5cm}
%\begin{column}{.45\textwidth}
%
%Assume there are two polytopes:
%\begin{columns}
%\column{.45\textwidth}
%\begin{figure}
%\includegraphics[width=1.3\textwidth]{figures-new/points.pdf}
%\caption{ {\ssmall $\{ \bar{V}^{\pi}: \pi \in \Pi \}$ is not given at the beginning}}
%\end{figure}
%\column{.45\textwidth}
%\begin{figure}
%\begin{tikzpicture}
%\node[] (img) at (-15.9,8.3){\includegraphics[width=1.2\textwidth]{figures/Lambda.pdf}};
%\node [align=left] at (-17.3, 9.7) {$\lambda_2$};
%\node [align=left] at (-16., 6.8) {$\lambda_1$};
%\end{tikzpicture}
%\caption{ {\ssmall Unknown weight vectors} }
%\end{figure}
%\end{columns}
%
%\end{column}
